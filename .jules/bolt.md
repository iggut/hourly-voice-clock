...
## 2023-10-31 - Hoisting DateTime Formatter Calls out of 1-Second Ticks
**Learning:** In Jetpack Compose ViewModels with high-frequency loops (e.g., 1-second ticks), recreating strings and parsing complex Date/Time properties repeatedly leads to heavy garbage collection pressure. Additionally, using just `dayOfYear * 24 + hour` for a hash will cause a collision across years (e.g., this year vs next year).
**Action:** When caching formatted date strings in ViewModels, extract the string formatting to variables and re-evaluate only when the underlying logical period (like the day or minute) changes. Ensure temporal hashes include the year using a large enough multiplier (e.g. `(year * 400 + dayOfYear)`) to prevent cross-year leap-year collisions.
