...
## 2026-07-26 - Isolate rapidly updating state to prevent full screen recomposition
**Learning:** In Jetpack Compose, passing a rapidly updating field (like `seconds`) as part of a larger state object (like `TimeDisplayState`) to a top-level composable causes the entire screen to recompose on every tick.
**Action:** Isolate the rapidly updating field into its own `StateFlow` and collect it within a dedicated, localized Composable (e.g. `SecondsDisplay`) to restrict recomposition to only the component that actually changes. Add primitive caching (e.g. `minuteHash`) in the ViewModel to prevent redundant string formatting allocations for the slower-updating fields.
