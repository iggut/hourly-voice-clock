...
## 2024-11-20 - Prevent `DateTimeFormatter` Allocations with Object Copying
**Learning:** In high-frequency 1-second view model ticker loops, repeatedly calling `LocalDateTime.format()` for values that rarely change (like hours and minutes) creates significant and unnecessary garbage collection overhead on every frame, even when seconds are cached.
**Action:** When updating a composite state object (like `TimeDisplayState`) in a loop, calculate a fast collision-free primitive hash (e.g., `now.hour * 60 + now.minute`) for the expensive-to-format parts. If the hash hasn't changed, use a fast `data class copy()` to update only the rapidly changing parts (like cached seconds strings) rather than re-evaluating the entire string format.
