...
## 2026-07-27 - View Model High Frequency Object Allocation & Recomposition Avoidance
**Learning:** In Jetpack Compose and ViewModels, updating a rapidly changing state (like seconds changing 60 times a minute, or high-frequency ticking) as part of a larger UI state object (e.g. `TimeDisplayState` holding full date/time) forces formatting of all parts and recomposes the entire view that observes that state every tick.
**Action:** Extract frequently changing state strings (like `seconds`) into their own `StateFlow` and collect it within a dedicated isolated Composable (`SecondsDisplay`). In the ViewModel, calculate a primitive hash for the slower-changing components (e.g. `timeHash = hour * 60 + minute`, `dateHash = epochDay`) and only re-format and emit new string resources for those larger state blocks when their hash actually changes.
