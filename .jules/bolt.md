...
## 2026-06-21 - Relative Time Hashing vs Date Allocations
**Learning:** In high-frequency loop optimizations (like 1-second view model ticks), hashing time components to detect changes is faster than re-allocating formatting strings. However, naive hashes like `year * 365 + dayOfYear` introduce leap-year collisions. Furthermore, attempting to avoid seconds allocations by using `padStart()` continuously generates garbage collection overhead, whereas using primitive lookup hashes across precomputed static arrays is robust and significantly reduces allocations.
**Action:** When extracting time string generation from tick loops into cached states, use combined variable hashes like `year * 400 + dayOfYear` and pre-calculated caches for high-frequency attributes like seconds to eliminate frame-rate string allocation completely.
