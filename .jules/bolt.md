...
## 2026-07-25 - View Model Date/Time UI String Formatting Optimization
**Learning:** In high-frequency loops (like updating the UI every second), calling methods that allocate new strings for the time (except seconds) and date when they haven't changed puts unnecessary pressure on garbage collection.
**Action:** Prevent redundant string formatting by caching the last processed date and minute hashes, and only executing the full formatting logic when those underlying values actually change. Update only the changing part (e.g. seconds) using a copy operation.
