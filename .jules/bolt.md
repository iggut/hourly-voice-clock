...
## 2024-05-18 - Optimize high-frequency string formatting in ViewModels
**Learning:** In ViewModel loops that tick rapidly (e.g., every second), evaluating date/time strings using `DateTimeFormatter` on every tick causes unnecessary object allocations and CPU overhead, because components like date or hour only change infrequently.
**Action:** Use primitive math to compute a combined hash of the time components (e.g., `now.hour * 60 + now.minute`) and cache the resulting formatted string, only re-evaluating when the hash changes. Extract high-frequency components (like seconds) so they can be updated independently without re-formatting the entire state.
