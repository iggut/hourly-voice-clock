...
## 2024-11-20 - Toggleable Row Wrappers
**Learning:** In Compose, wrapping a `Switch` in a `.clickable()` `Row` causes screen readers to misinterpret the interaction. The visual text label doesn't naturally merge with the toggle's semantics, meaning users may not hear the correct "on/off" state when focusing on the row.
**Action:** Always use `.toggleable(value = state, onValueChange = { ... }, role = Role.Switch)` on the container `Row` instead of `.clickable()`, and set the inner `Switch`'s `onCheckedChange` to `null`. This ensures the entire row is announced as a single, accessible toggle switch with the correct state.
