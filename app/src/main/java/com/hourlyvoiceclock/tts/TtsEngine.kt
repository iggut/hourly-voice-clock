...
class AndroidTtsEngine(context: Context) : TtsEngine {
    ...
    override fun getEngines(): List<TtsEngineInfo> {
        ...
        // Build the combined list directly to avoid intermediate list allocations and copies
        val result = ArrayList<TtsEngineInfo>()
        val existingPackageNames = HashSet<String>()

        for (engine in ttsInstance.engines) {
            result.add(
                TtsEngineInfo(
    ...
    private fun inferGender(name: String): String? {
        ...
        // ⚡ Bolt: Replace Regex split with index-based parsing to avoid list/regex allocations
        var start = 0
        while (start < lower.length) {
            ...
            if (end > start) {
                // ⚡ Bolt: substring() allocates a small string for O(1) map lookup
                val segment = lower.substring(start, end)
